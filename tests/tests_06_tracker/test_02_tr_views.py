...
def test_TR005_total_values_appear_on_list_page(user_transactions, client):
...
@pytest.mark.django_db
def test_TR006_transaction_type_filter(user_transactions, client):
    user = user_transactions[0].user
    client.force_login(user)
...
@pytest.mark.django_db
def test_TR007_start_end_date_filter(user_transactions, client):
    user = user_transactions[0].user
    client.force_login(user)
...
@pytest.mark.django_db
def test_TR008_ategory_filter(user_transactions, client):
    user = user_transactions[0].user
    client.force_login(user)
...
@pytest.mark.django_db
def test_TR009_add_transaction_request(user, transaction_dict_params, client):
    client.force_login(user)
...
@pytest.mark.django_db
def test_TR010_cannot_add_transaction_with_negative_amount(
...
@pytest.mark.django_db
def test_TR011_update_transaction_request(user, transaction_dict_params, client):
    client.force_login(user)
...
@pytest.mark.django_db
def test_TR012_delete_transaction_request(user, transaction_dict_params, client):
    client.force_login(user)
    assert Transaction.objects.filter(user=user).count() == 1
    transaction = Transaction.objects.first()
